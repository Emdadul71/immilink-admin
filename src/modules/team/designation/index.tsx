...
const TeamDesignationList = () => {
  ...
  const [designationId, {}] = useLazyGetSingleTeamDesignationQuery();

  const { data, isFetching } = useGetAllTeamDesignationQuery({
    name: keyword,
    page,
    limit,
  });
  ...
  const [deletId] = useDeleteTeamDesignationMutation();
  ...
  const columns: ColumnsType<DataType> = [
...

    {
      ...
      dataIndex: "slug",
      render: (_, record) => (
        <span className="line-clamp-1">{record?.slug}</span>
      ),
    },
    {
...
export default TeamDesignationList;
